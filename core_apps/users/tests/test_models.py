...
# this makes sure the test has access to the db, handles setup and teardown
...
def test_update_user(normal_user):
    new_first_name = 'Julio'
    new_last_name = 'Iglesias'
...
def test_superuser_is_not_staff(user_factory):
    ...
    assert str(err.value) == 'Super user must have is_staff set to True.'
    
@pytest.mark.django_db
def test_superuser_is_not_superuser(user_factory):
    with pytest.raises(ValueError) as err:
        user_factory.create(is_superuser=False, is_staff=True)
    assert str(err.value) == 'Super user must have is_superuser set to True.'
